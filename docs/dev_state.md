...
- **ClimbToAltitude closed-loop (fixed 2026-08-08, SIL verified same day):** reads `/fmu/out/vehicle_local_position_v1.z` (NED → height = `-z`), SUCCESS within `tolerance_m`, timeout → FAILURE, skips climb when already at target band (BT relaunch safe).
...
- **Orbit search + BT acquire fix:** `7f5a1cd` on `main` — `feat(state-machine)`; Main PC SIL verified 2026-08-08
